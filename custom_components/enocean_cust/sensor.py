"""Support for Enocean Cust sensors."""
...
from .entity import EnoceanCustEntity
...
DEFAULT_NAME = "Enocean Cust sensor"
...
class EnoceanCustSensorEntityDescription(SensorEntityDescription):
    """Describes Enocean Cust sensor entity."""

    unique_id: Callable[[list[int]], str | None]


SENSOR_DESC_TEMPERATURE = EnoceanCustSensorEntityDescription(
...
SENSOR_DESC_HUMIDITY = EnoceanCustSensorEntityDescription(
...
SENSOR_DESC_POWER = EnoceanCustSensorEntityDescription(
...
SENSOR_DESC_WINDOWHANDLE = EnoceanCustSensorEntityDescription(
...
def setup_platform(
    ...
    """Set up an Enocean Cust sensor device."""
    dev_id: list[int] = config[CONF_ID]
    dev_name: str = config[CONF_NAME]
    sensor_type: str = config[CONF_DEVICE_CLASS]

    entities: list[EnoceanCustSensor] = []
    if sensor_type == SENSOR_TYPE_TEMPERATURE:
        ...
        entities = [EnoceanCustHumiditySensor(dev_id, dev_name, SENSOR_DESC_HUMIDITY)]

    elif sensor_type == SENSOR_TYPE_POWER:
        entities = [EnoceanCustPowerSensor(dev_id, dev_name, SENSOR_DESC_POWER)]

    elif sensor_type == SENSOR_TYPE_WINDOWHANDLE:
        entities = [EnoceanCustWindowHandle(dev_id, dev_name, SENSOR_DESC_WINDOWHANDLE)]

    add_entities(entities)


class EnoceanCustSensor(EnoceanCustEntity, RestoreSensor):
    """Representation of an  Enocean Cust sensor device such as a power meter."""

    def __init__(
        self,
        dev_id: list[int],
        dev_name: str,
        description: EnoceanCustSensorEntityDescription,
    ) -> None:
        """Initialize the Enocean Cust sensor device."""
...
class EnoceanCustPowerSensor(EnoceanCustSensor):
    """Representation of an Enocean Cust power sensor.
...
class EnoceanCustTemperatureSensor(EnoceanCustSensor):
    """Representation of an Enocean Cust temperature sensor device.
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the Enocean Cust temperature sensor device."""
...
class EnoceanCustHumiditySensor(EnoceanCustSensor):
    """Representation of an Enocean Cust humidity sensor device.
...
class EnoceanCustWindowHandle(EnoceanCustSensor):
    """Representation of an Enocean Cust window handle device.
