"""Support for Enocean Cust switches."""
...
DEFAULT_NAME = "Enocean Cust Switch"
...
async def async_setup_platform(
    ...
    """Set up the Enocean Cust switch platform."""
    channel: int = config[CONF_CHANNEL]
    dev_id: list[int] = config[CONF_ID]
    dev_name: str = config[CONF_NAME]

    _migrate_to_new_unique_id(hass, dev_id, channel)
    async_add_entities([EnoceanCustSwitch(dev_id, dev_name, channel)])


class EnoceanCustSwitch(EnoceanCustEntity, SwitchEntity):
    """Representation of an Enocean Cust switch device."""

    _attr_is_on = False

    def __init__(self, dev_id: list[int], dev_name: str, channel: int) -> None:
        """Initialize the Enocean Cust switch device."""
